...
use baseview::{
    DropData, DropEffect, Event, EventStatus, PhySize, Window, WindowHandle, WindowHandler,
        ...
        match &event {
            baseview::Event::Mouse(event) => match event {
                ...
                | baseview::MouseEvent::DragMoved {
                    ...
                    self.pointer_pos_in_points = Some(pos);
                    self.egui_input.events.push(egui::Event::PointerMoved(pos));
                    self.egui_input.hovered_files = hovered_files_from_drop_data(data);

                    if has_file_drop(data) {
                ...
                baseview::MouseEvent::DragDropped {
                    ...
                    self.egui_input
                        .dropped_files
                        .extend(dropped_files_from_drop_data(data));
...
fn has_file_drop(data: &DropData) -> bool {
    matches!(data, DropData::Files(paths) if !paths.is_empty())
}

fn hovered_files_from_drop_data(data: &DropData) -> Vec<egui::HoveredFile> {
    match data {
        DropData::Files(paths) => paths
            .iter()
...
fn dropped_files_from_drop_data(data: &DropData) -> Vec<egui::DroppedFile> {
    match data {
        DropData::Files(paths) => paths
            .iter()
