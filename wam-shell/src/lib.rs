...
pub struct WasmProcessor {
    ...
    sidechain_buffers: [Vec<f32>; MAX_CHANNELS],
}
...
impl WasmProcessor {
    fn new(
        ...
        dsp.prepare(sample_rate, main_channels, sidechain_channels);
        Self {
            dsp,
            ...
            sidechain_buffers: std::array::from_fn(|_| vec![0.0; max_block]),
        }
    }
    ...
    fn process(
        ...
        match (
            ...
            (1, 1) => {
                let mut main: [&mut [f32]; 1] = [&mut self.main_buffers[0][..frames]];
                let sc: [&mut [f32]; 1] = [&mut self.sidechain_buffers[0][..frames]];
                self.dsp.process_block(&mut main, Some(&sc), params);
            }
            (1, 2) => {
                let mut main: [&mut [f32]; 1] = [&mut self.main_buffers[0][..frames]];
                let (sc0, sc_rest) = self.sidechain_buffers.split_at_mut(1);
                let sc: [&mut [f32]; 2] = [&mut sc0[0][..frames], &mut sc_rest[0][..frames]];
                self.dsp.process_block(&mut main, Some(&sc), params);
            }
            (2, 1) => {
                let (left, rest) = self.main_buffers.split_at_mut(1);
...
pub unsafe extern "C" fn sf_process(
    ...
    processor.process(input_ptr, output_ptr, sidechain_ptr, frames as usize)
}
